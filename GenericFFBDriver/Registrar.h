...
protected:
	...
	BOOL SetInRegistryDWord(HKEY hRootKey, LPCSTR subKey, LPCSTR keyName, DWORD keyValue)
	{
		HKEY hKeyResult;
		DWORD dwDisposition;
		if (RegCreateKeyExA(hRootKey, subKey, 0, NULL, REG_OPTION_NON_VOLATILE,
		...
		DWORD retVal = RegSetValueExA(hKeyResult, keyName, 0, REG_DWORD, (const BYTE*)&keyValue, sizeof(DWORD));
		RegCloseKey(hKeyResult);
		return (retVal == ERROR_SUCCESS) ? TRUE : FALSE;
	}
...
public:
	bool RegisterObject(REFIID riid, LPCSTR LibId, LPCSTR ClassId, LPCSTR Path)
	{
		...
		dData[0] = 0xe8;
		dData[1] = 0x03;
		DWORD amplifyValue = 0x03e8; 
		if (!SetInRegistryDWord(HKEY_LOCAL_MACHINE, root, "Amplify", amplifyValue))
			return false;
		...
		// AXES ----------------	
		auto axePath = ROOT_REGISTRY_KEY "\\Axes";
		byte axeAttrData[] = {
		...
		// Buttons -------------
		auto btnPath = ROOT_REGISTRY_KEY "\\Buttons";
		byte btnData[] = {
		...
		// ForceFeedback -------
		auto oemPath = ROOT_REGISTRY_KEY "\\OEMForceFeedback";
		...
		if (!mkffbkey(0, 0x00, 0x00, 0x01, 0x86, 0xed, 0xed, oemPath, "Constant")) return false;
		if (!mkffbkey(1, 0x01, 0x00, 0x02, 0x86, 0xef, 0xef, oemPath, "Ramp Force")) return false;
		if (!mkffbkey(2, 0x02, 0x00, 0x03, 0x86, 0xef, 0xef, oemPath, "Square Wave")) return false;
		if (!mkffbkey(3, 0x03, 0x00, 0x03, 0x86, 0xef, 0xef, oemPath, "Sine Wave")) return false;
		if (!mkffbkey(4, 0x04, 0x00, 0x03, 0x86, 0xef, 0xef, oemPath, "Triangle Wave")) return false;
		if (!mkffbkey(5, 0x05, 0x00, 0x03, 0x86, 0xef, 0xef, oemPath, "Sawtooth Up Wave")) return false;
		if (!mkffbkey(6, 0x06, 0x00, 0x03, 0x86, 0xef, 0xef, oemPath, "Sawtooth Down Wave")) return false;
		...
		if (!mkffbkey(11, 0x00, 0x01, 0x05, 0x86, 0xef, 0xef, oemPath, "Custom Force")) return false;
