...
#define ACTUATOR_SMALL 0x01000202
#define ACTUATOR_BIG 0x01000502
	...
	void VibrationController::VibrationThreadEntryPoint(DWORD dwID)
	{
		...
		while (true) {
			...
			if (forceX != lastForceX || forceY != lastForceY) {
#ifdef _DEBUG
				LogMessage("Sending rumble command: dwId:0x%lx, forceX:%u, forceY:%u", dwID, (forceX * 100.0f) / 254.0f, (forceY * 100.0f) / 254.0f);
#endif
				// Send the command
				...
				SendVibrationForce(hHidDevice[dwID], lastForceX, lastForceY, dwID);
			}
				lastForceX = forceX;
				lastForceY = forceY;
			}
			...
			for (size_t i = 0; i < lpEffect->cAxes; i++)
			{
				if (!hasAxisRefs) {
					...
					case ACTUATOR_SMALL:
						...
						LogMessage(" Small actuator actuator  referenced. Provided: 0x%08lx, small: 0x%08lx, big: 0x%08lx",
							lpEffect->rgdwAxes[i], ACTUATOR_SMALL, ACTUATOR_BIG);
						break;
					case ACTUATOR_BIG:
						bigActuator = true;

						LogMessage(" Big actuator actuator  referenced. Provided: 0x%08lx, small: 0x%08lx, big: 0x%08lx",
							lpEffect->rgdwAxes[i], ACTUATOR_SMALL, ACTUATOR_BIG);
						break;
